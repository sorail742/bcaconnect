...
describe("🛡️ Suite de Tests de Sécurité - Authentification (V2.6)", () => {
  ...
  describe("POST /api/auth/register", () => {
    ...
    it("doit bloquer une inscription technicien sans spécialités ni zone d'intervention", async () => {
      const res = await request(app).post("/api/auth/register").send({
        nom_complet: "Technicien Test",
        email: "tech-test@bca.gn",
      ...
      expect(errorMessages).toMatch(/Spécialités|zone d'intervention/i);
    });

    it("doit réussir une inscription technicien valide", async () => {
      const res = await request(app).post("/api/auth/register").send({
        nom_complet: "Technicien Valide",
        email: "tech-valide@bca.gn",
      ...
      expect(res.body.user).toHaveProperty("role", "technicien");
    });
  ...
  describe("🛡️ Test Chiffrement AES (P0 #5)", () => {
    it("doit chiffrer le numéro de téléphone en base de données via les hooks", async () => {
      ...
      const [results] = await sequelize.query(
        `SELECT telephone FROM utilisateurs WHERE email = 'hook@test.gn'`,
      );
