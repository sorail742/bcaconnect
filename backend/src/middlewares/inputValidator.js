...
const validateRegister = [
  ...
  body("mot_de_passe")
    .isLength({ min: 8 })
    .withMessage(
      "Le mot de passe doit faire au moins 8 caractères (Sécurité P0).",
    )
    .matches(/\d/)
...
const validateCreditRequest = [
  ...
  body("motif")
    .notEmpty()
    .withMessage("Le motif du financement est obligatoire pour l'analyse IA."),
  validateRequest,
];
...
const validateOrderCreate = [
  ...
  body("items.*.quantite")
    .isInt({ min: 1 })
    .withMessage("Quantité minimum : 1."),
  validateRequest,
];
