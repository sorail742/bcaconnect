/**
 * Copie backend — profils d'attributs BCA Connect (38 catégories Alibaba-style).
 * Source de référence : frontend/src/category/lib/categoryAttributeProfiles.js
 * (identique à l'export CJS près) — dupliquée ici car le backend est
 * déployé/conteneurisé indépendamment du frontend (Render "Root Directory:
 * backend", docker-compose.yml build: ./backend) et ne peut pas importer un
 * chemin relatif hors de son propre répertoire. Toute modification du
 * contenu métier doit être répercutée dans les deux fichiers.
 */
