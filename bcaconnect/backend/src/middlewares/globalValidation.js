...
const validatePagination = (req, res, next) => {
    ...
    if (page < 1) {
        return sendValidationError(res, 'page', 'page doit être supérieur à 0', 'query');
    }

    if (limit < 1 || limit > 100) {
        return sendValidationError(res, 'limit', 'limit doit être entre 1 et 100', 'query');
    }
        ...
        if (monetaryFields.some(field => key.toLowerCase().includes(field))) {
            ...
            if (typeof value === 'number') {
                if (value.toString().split('.')[1]?.length > 2) {
                    return sendValidationError(res, key, `${key} ne peut avoir plus de 2 décimales`);
                }

                if (value < 0 || value > 9999999999.99) {
                    return sendValidationError(res, key, `${key} doit être entre 0 et 9,999,999,999.99`);
                }
            }
        }
    }
        ...
        if (dateFields.some(field => key.toLowerCase().includes(field))) {
            ...
            if (value && typeof value === 'string') {
                ...
                if (isNaN(date.getTime())) {
                    return sendValidationError(res, key, `${key} doit être au format ISO 8601`);
                }
                ...
                if (date < maxPastDate || date > maxFutureDate) {
                    return sendValidationError(res, key, `${key} doit être une date raisonnable`);
                }
            }
        }
    }
    ...
    for (const key of Object.keys(req.body || {})) {
        if (enums[key] && req.body[key]) {
            if (!enums[key].includes(req.body[key])) {
                return sendValidationError(res, key, `Valeur invalide pour ${key}. Attendu: ${enums[key].join(', ')}`);
            }
        }
    }
    ...
    for (const key of Object.keys(req.body || {})) {
        if (arrayFields.some(field => key.toLowerCase().includes(field))) {
            ...
            if (Array.isArray(value) && value.length > 1000) {
                return sendValidationError(res, key, `${key} trop volumineux (max 1000)`);
            }
        }
    }
...
const validateNestedObjects = (req, res, next) => {
    ...
    if (!validateObject(req.body)) {
        return sendValidationError(res, 'body', 'Profondeur d\'objet excessive (max 5)');
    }
