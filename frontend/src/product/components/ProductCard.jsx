        ...
        <div className="flex flex-col gap-0.5 overflow-hidden">
            <div className="flex items-baseline gap-1 flex-wrap">
                ...
                <span className={cn(
                    ...
                    isBca ? "text-[#1CA0DB] text-xs" : "font-black text-primary",
                ...
                {[1, 2, 3, 4, 5].map((s) => (
                    <Star 
                        ...
                        className={cn(
                            ...
                            s <= Math.round(rating) ? "fill-[#1CA0DB] text-[#1CA0DB]" : "fill-[#e8e8e8] text-[#e8e8e8]"
                        )} 
                    />
                ))}
            </div>
...
const ProductCard = ({ 
    ...
    const handleChat = async (e) => {
        ...
        try {
            ...
            messageService.shareProduct(vendorId, product).catch(() => {});
            ...
            <tr className="hover:bg-slate-50 transition-colors border-b border-slate-100">
                ...
                <td className="px-6 py-4 text-right">
                   <div className="flex items-center justify-end gap-2">
                        {onManageVariants && (
                            <button
                                onClick={(e) => { e.preventDefault(); onManageVariants(product); }}
                                title="Variantes"
                                className={cn(
                                    "size-8 rounded border flex items-center justify-center transition-all",
                                    product.has_variants
                                        ? "border-[#1CA0DB] text-[#1CA0DB] bg-[#eff6ff]"
                                        : "border-[#e8e8e8] text-[#666] hover:text-[#1CA0DB] hover:border-[#1CA0DB]"
                                )}
                            >
                                <Layers className="size-3.5" />
                            </button>
                        )}
                        <button onClick={(e) => { e.preventDefault(); onEdit?.(product); }} className="size-8 rounded border border-[#e8e8e8] flex items-center justify-center text-[#666] hover:text-[#1CA0DB] hover:border-[#1CA0DB] transition-all">
        ...
        return (
            <Link to={`/product/${product.id}`} className="bca-product-card flex flex-row h-full group">
                ...
                <div className="flex-1 p-4 flex flex-col min-w-0">
                    ...
                    <h3 className="text-sm font-semibold text-[#333] line-clamp-2 mb-2 group-hover:text-[#1CA0DB] transition-colors">
                        {product.nom_produit}
                    </h3>
        ...
        <Link to={`/product/${product.id}`} className="bca-product-card flex flex-col h-full group">
            <div className="relative aspect-square overflow-hidden bg-[#fafafa]">
                ...
                <button 
                    ...
                    className="absolute top-2 right-2 p-1.5 bg-white/95 rounded-sm border border-[#e8e8e8] hover:border-[#1CA0DB] transition-colors"
                >
            ...
            <div className="p-3 flex flex-col flex-1 border-t border-[#f0f0f0]">
                <SupplierRow product={product} />
                <h3 className="text-xs sm:text-sm font-medium text-[#333] line-clamp-2 min-h-[2.5rem] mb-2 leading-snug group-hover:text-[#1CA0DB] transition-colors">
                    {product.nom_produit}
                </h3>
