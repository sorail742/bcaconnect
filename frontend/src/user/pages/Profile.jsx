...
import { useMyStore } from '../../shop/hooks/useVendorData';
...
import {
    ...
    Activity, ArrowRight, Bell, Mail, Camera, Loader2, Store as StoreIcon
} from 'lucide-react';
...
const UserProfile = () => {
    ...
    const isVendor = displayUser?.role === 'fournisseur';
    const { data: myStore } = useMyStore({ enabled: isVendor });
    ...
    const handleFileChange = async (e) => {
        ...
        if (file.size > 5 * 1024 * 1024) {
            toast.error("Image trop lourde (5MB max).");
            return;
        }
    ...
    const { mutateAsync: deleteAccountMutation } = useApiMutation(
        (confirmationText) => deleteAccount(confirmationText),
        {
    ...
    const { mutate: disable2FAMutation } = useApiMutation(
        () => authService.disable2FA(),
        {
            successMessage: "Authentification 2FA désactivée.",
            invalidateKeys: ['user-profile'],
            onSuccess: async () => {
                if (refreshUser) await refreshUser();
            }
    ...
    const handleDelete = () => setShowDeleteAccountConfirm(true);
    ...
    return (
        <DashboardLayout title={activeTab === 'profile' ? t('profTitle') : t('profSettings')}>
                        ...
                        <div className="glass-card border border-white/20 rounded-2xl overflow-hidden shadow-sm">
                            ...
                            <div className="px-5 pb-5 -mt-10 relative">
                                <div className="relative w-fit">
                                    ...
                                    {isVendor && myStore?.logo_url && (
                                        <div
                                            className="absolute -top-1 -left-1 size-8 rounded-lg border-2 border-card bg-white shadow-sm overflow-hidden flex items-center justify-center"
                                            title={myStore.nom_boutique}
                                        >
                                            <img src={myStore.logo_url} alt="" className="w-full h-full object-contain p-0.5" />
                                        </div>
                                    )}
                                    <button
                                        ...
                                        <div className="flex items-center gap-1.5">
                                            {myStore?.is_verified ? (
                                                <>
                                                    ...
                                                    <span className="text-xs text-primary font-medium">{t('profCertifiedMerchant')}</span>
                                                </>
                                            ) : (
                                                <>
                                                    <ShieldAlert className="size-3.5 text-amber-500" />
                                                    <span className="text-xs text-amber-600 font-medium">{t('profPendingCertification')}</span>
                                                </>
                                            )}
                                        </div>
                                    ) : (
                                        <div className="flex items-center gap-1.5">
                                            <BadgeCheck className={cn("size-3.5", displayUser?.statut === 'actif' ? "text-primary" : "text-muted-foreground")} />
                                            <span className={cn("text-xs font-medium", displayUser?.statut === 'actif' ? "text-primary" : "text-muted-foreground")}>
                                                {displayUser?.statut === 'actif' ? t('profActiveMember') : t('profVerifiedMember')}
                                            </span>
                                        </div>
                                    )}
                                    {isVendor && myStore && (
                                        ...
                                        <div className="flex items-center justify-between p-4 bg-muted rounded-xl border border-border hover:border-primary/30 transition-colors relative z-10">
                                            ...
                                            <div className="flex items-center gap-2">
                                                {user?.two_factor_enabled ? (
                                                    <>
                                                        <span className="h-9 px-4 rounded-xl text-xs font-bold bg-emerald-500/10 text-emerald-500 border border-emerald-500/20 flex items-center">
                                                            ✓ {t('prof2FASecure')}
                                                        </span>
                                                        <button
                                                            onClick={handleDisable2FA}
                                                            className="h-9 px-4 rounded-xl text-xs font-bold bg-rose-500/10 text-rose-500 border border-rose-500/20 hover:bg-rose-500 hover:text-white transition-all"
                                                        >
                                                            Désactiver
                                                        </button>
                                                    </>
                                                    ...
                                                    <button
                                                        ...
                                                        className="h-9 px-5 rounded-xl text-xs font-bold bg-primary text-primary-foreground hover:bg-primary/90 shadow-lg shadow-primary/20 transition-all"
                                                    >
                                                        {t('prof2FASetup')}
                                                    </button>
                                                )}
                ...
                <div className="p-8 space-y-8 text-center">
                    ...
                    <div className="space-y-4">
                        ...
                        {twoFactorData?.backupCodes && (
                            <div className="mt-4 p-4 bg-red-50 dark:bg-red-900/10 border border-red-200 dark:border-red-900/30 rounded-xl text-left">
                                <p className="text-xs font-bold text-red-600 dark:text-red-400 uppercase mb-2">
                                    ⚠️ Codes de secours (À conserver)
                                </p>
                                <div className="grid grid-cols-2 gap-2 text-xs font-mono text-gray-700 dark:text-gray-300">
                                    {twoFactorData.backupCodes.map((code, index) => (
                                        <div key={index} className="bg-white dark:bg-black/20 px-2 py-1 rounded border border-red-100 dark:border-red-900/20">{code}</div>
                                    ))}
                                </div>
                            </div>
                        )}
            ...
            {/* Crop Modal */}
            <ModalOverlay open={showCropModal} onClose={() => setShowCropModal(false)} maxWidth="max-w-md">
                <div className="p-6 border-b border-border bg-muted/50 flex items-center justify-between">
                    <h3 className="text-sm font-bold text-foreground">Recadrer la photo</h3>
                    <button type="button" onClick={() => setShowCropModal(false)} className="size-8 rounded-lg bg-muted hover:bg-rose-500/10 hover:text-rose-500 flex items-center justify-center transition-colors">
                        <X className="size-4" />
                    </button>
                </div>
                <div className="p-6 space-y-6">
                    ...
                    <div className="space-y-2">
                        <label className="text-xs font-semibold text-muted-foreground uppercase tracking-wide">Zoom</label>
                        <input
                            type="range"
                            value={zoom}
                            min={1}
                    ...
                    <button
                        ...
                        className="w-full h-12 bg-primary text-primary-foreground hover:bg-primary/90 rounded-2xl font-bold text-sm transition-all flex items-center justify-center gap-2"
                    >
                        <CheckCircle2 className="size-5" /> Enregistrer la photo
                    </button>
                </div>
            </ModalOverlay>
            ...
            <ConfirmDeleteModal
                ...
                itemName={displayUser?.email}
                itemLabel="votre compte"
                description="Votre compte sera anonymisé conformément au RGPD : nom, e-mail et téléphone seront effacés et vous serez déconnecté. Un historique interne est conservé pour l'équipe admin en cas de litige, mais vous perdrez tout accès."
                confirmButtonText="Supprimer mon compte"
                onConfirm={(confirmationText) => deleteAccountMutation(confirmationText)}
            />
        </DashboardLayout>
    );
};
