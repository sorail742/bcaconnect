    ...
    <DashboardLayout title="Espace Technicien">
      <div className="p-4 md:p-8 max-w-7xl mx-auto space-y-8 animate-in fade-in duration-500">
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
          ...
          <button
            onClick={() => navigate('/technician/wallet')}
            className="flex items-center gap-2 bg-primary text-primary-foreground px-4 py-2 rounded-xl font-medium shadow-lg hover:shadow-primary/25 hover:-translate-y-0.5 transition-all"
          >
          ...
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
            ...
            <div className="flex items-center p-1 bg-muted rounded-lg">
              {['all', 'pending', 'completed'].map((tab) => (
                ...
                >
                  {tab === 'all' ? 'Toutes' : tab === 'pending' ? 'Actives' : 'Terminées'}
                </button>
              ))}
            </div>
          </div>
