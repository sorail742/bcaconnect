        ...
        <div className="bg-white dark:bg-[#0A0D14] min-h-screen text-slate-900 dark:text-foreground font-jakarta">
            <div className="max-w-5xl mx-auto px-6 pt-44 pb-32 space-y-24 animate-in fade-in slide-in-from-bottom-12 duration-1000">
                ...
                <div className="text-center space-y-8 py-16 relative">
                    <div className="absolute top-0 left-1/2 -translate-x-1/2 size-[50rem] bg-[#1CA0DB]/5 rounded-full blur-[150px] -z-10 mix-blend-screen pointer-events-none" />
                    <div className="flex flex-col items-center gap-6">
                        <div className="flex items-center gap-4">
                            <div className="size-2 rounded-full bg-[#1CA0DB] animate-pulse shadow-[0_0_15px_rgba(28,160,219,0.5)]" />
                            <span className="text-[10px] font-black text-[#1CA0DB] uppercase   leading-none pt-0.5">{lang === 'FR' ? "CADRE JURIDIQUE" : "LEGAL FRAMEWORK"}</span>
                        </div>
                        <h1 className="text-2xl md:text-2xl lg:text-xl font-semibold  tracking-tighter text-slate-900 dark:text-foreground uppercase leading-none mb-4">
                            {t('termsTitle').split(' ').slice(0, -1).join(' ')} <br /> <span className="text-[#1CA0DB]">{t('termsTitle').split(' ').slice(-1)}</span>
                        </h1>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                    {[
                        { icon: BadgeCheck, label: lang === 'FR' ? "QUALITÉ GARANTIE" : "GUARANTEED QUALITY", desc: lang === 'FR' ? "Vérification systématique des vendeurs." : "Systematic verification of sellers.", color: "text-[#1CA0DB]", bg: "bg-[#1CA0DB]/10" },
                        ...
                        <div key={i} className="p-8 rounded-3xl bg-white dark:bg-[#0F1219] border-2 border-slate-50 dark:border-foreground/5 space-y-6 shadow-xl hover:border-[#1CA0DB]/20 transition-all duration-500">
                ...
                <div className="space-y-16 py-12">
                     <section className="space-y-6">
                        <div className="flex items-center gap-4">
                            <div className="size-8 rounded-xl bg-slate-50 dark:bg-foreground/5 border-2 border-slate-50 dark:border-foreground/5 flex items-center justify-center text-[#1CA0DB]">
                        ...
                        <div className="text-muted-foreground font-bold uppercase  text-[10px] leading-relaxed  border-l-4 border-[#1CA0DB]/20 pl-8 opacity-80">
                     ...
                     <section className="space-y-8">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 pl-8">
                            <div className="space-y-4">
                                <p className="text-[10px] font-black text-[#1CA0DB] uppercase tracking-widest">{t('roleSeller').split(' ')[0]}</p>
                                <p className="text-[10px] text-muted-foreground font-bold uppercase tracking-widest  leading-relaxed">
                                    {lang === 'FR' ? "Garantir l'authenticité des produits." : "Guarantee product authenticity."}
                                </p>
                            </div>
                            <div className="space-y-4">
                                <p className="text-[10px] font-black text-[#1CA0DB] uppercase tracking-widest">{t('roleBuyer').split(' ')[0]}</p>
                                <p className="text-[10px] text-muted-foreground font-bold uppercase tracking-widest  leading-relaxed">
