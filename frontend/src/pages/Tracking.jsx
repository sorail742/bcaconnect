...
const DeliveryTracking = () => {
    ...
    const [trackingData, setTrackingData] = useState(null); // Données publiques retournées par l'API
    ...
    const handleSearch = async (id = trackingNumber) => {
        ...
        setSearchError(null);
        try {
            // Utilise le nouvel endpoint PUBLIC (pas besoin d'être connecté)
            const data = await deliveryService.trackOrder(targetId);
            setTrackingData(data);
        } catch (err) {
            setTrackingData(null);
            const message = err.response?.data?.message || 'Commande introuvable ou erreur de chargement.';
    ...
    }, [on, off, trackingData?.id]);

    // Simulate Live GPS Movement when in transit
    useEffect(() => {
        const isEnCours = trackingData?.statut_livraison === 'en_cours' || trackingData?.statut === 'en_cours';
        if (!isEnCours) return;
                    ...
                    <div className="mt-6 font-jakarta min-h-[400px]">
                        ...
                        >
                            ...
                            <div className="space-y-6">
                                ...
                                {(trackingData.statut_livraison === 'en_cours' || trackingData.statut === 'en_cours') && (
                                    <div className="bg-card border border-border rounded-[2rem] p-6 shadow-2xl relative overflow-hidden flex flex-col items-center text-center space-y-4">
                                        ...
                                        <p className="text-[10px] text-muted-foreground font-medium relative z-10">Mise à jour en direct du véhicule...</p>
                                    </div>
                                )}
