...
const carrierIcon = new L.DivIcon({
    html: `<div class="bg-[#1CA0DB] rounded-full flex items-center justify-center shadow-lg border-2 border-white animate-pulse" style="width: 28px; height: 28px;"><svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="white" stroke-width="2"><path d="M10 17h4V5H2v12h3"/><circle cx="7.5" cy="17.5" r="2.5"/><circle cx="17.5" cy="17.5" r="2.5"/></svg></div>`,
    className: '',
    iconSize: [28, 28],
    iconAnchor: [14, 14]
});
...
const CarrierDashboard = () => {
    ...
    const handleAssign = async (orderId, vendorNames = []) => {
        if (vendorNames.length > 1) {
            const ok = window.confirm(
                `Cette commande regroupe ${vendorNames.length} boutiques distinctes (${vendorNames.join(', ')}). Vous devrez passer chez chacune avant la livraison finale. Continuer ?`
            );
            if (!ok) return;
        }
    ...
    const availableColumns = [
        {
            ...
            render: (row) => (
                <span className="font-black text-[#1CA0DB] uppercase text-[9px] tracking-wide bg-[#1CA0DB]/5 px-2 py-1 rounded-lg border border-[#1CA0DB]/10">
        ...
        },
        {
            ...
            render: (row) => {
                const vendorCount = row.vendor_count ?? row.vendors?.length ?? 0;
                const isMulti = vendorCount > 1;
                return (
                    <div className="flex flex-col gap-1">
                        <span className={cn(
                            "flex items-center gap-1.5 font-black text-[10px] uppercase",
                            isMulti ? "text-amber-500" : "text-slate-800 dark:text-foreground"
                        )}>
                            {isMulti ? <AlertTriangle className="size-3" /> : <Store className="size-3" />}
                            {isMulti ? `${vendorCount} boutiques à ramasser` : (row.vendors?.[0]?.nom_boutique || t('carSourceShop'))}
                        </span>
                        <span className="text-[8px] text-muted-foreground font-black uppercase tracking-widest truncate max-w-[180px]">
                            {isMulti
                                ? row.vendors.map(v => v.nom_boutique).join(' · ')
                                : (row.vendors?.[0]?.localisation || row.adresse_livraison?.split(',')[0] || 'ZONE N/A')}
                        </span>
                    </div>
                );
            }
        ...
        },
        {
            ...
            render: (row) => (
                <div className="text-right">
                    <button
                        onClick={() => handleAssign(row.id, (row.vendors || []).map(v => v.nom_boutique))}
                        disabled={assignMutation.isPending}
                        className="h-8 px-4 bg-[#1CA0DB] text-white text-[9px] font-black rounded-lg transition-all shadow-md active:scale-95 uppercase tracking-widest hover:brightness-110 flex items-center gap-2 ml-auto disabled:opacity-50"
                    >
        ...
        <DashboardLayout title={t('carLogCenter')}>
            <div className="space-y-8 animate-in fade-in duration-700 pb-24">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                    <DashboardCard title={t('carMyAssignments')} value={stats.assigned} icon={ClipboardList} className="h-32 border-[#1CA0DB]/10" />
                ...
                <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
                        ...
                        <div className="flex items-center gap-4 bg-white dark:bg-[#0F1219] p-2 rounded-2xl border border-slate-200 dark:border-foreground/5 shadow-sm">
                            <button
                                ...
                                className={cn(
                                    ...
                                    activeTab === 'AVAILABLE' 
                                        ? "bg-[#1CA0DB] text-white shadow-lg shadow-[#1CA0DB]/20" 
                    ...
                    <div className="lg:col-span-4 space-y-6">
                            ...
                            <div className="aspect-[4/3] rounded-2xl border border-slate-200 dark:border-slate-600 overflow-hidden relative z-10 shadow-inner bg-slate-100">
                                ...
                                >
                                    ...
                                    {routePoints.length > 1 && (
                                        <Polyline positions={routePoints} color="#1CA0DB" weight={3} opacity={0.8} />
                                    )}
                        ...
                        {/* Status Console */}
                        <div className="bg-[#1CA0DB]/5 border border-[#1CA0DB]/20 rounded-3xl p-6 space-y-4">
                            <div className="flex items-center gap-3">
                                <Shield className="size-5 text-[#1CA0DB]" />
                                <span className="text-[10px] font-black uppercase tracking-widest text-[#1CA0DB]">{t('carCarrierStatus')}</span>
                            </div>
                            ...
                            <div className="pt-2">
                                <button
                                    ...
                                    className="w-full h-12 bg-slate-900 dark:bg-white text-white dark:text-slate-900 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all hover:bg-[#1CA0DB] hover:text-white"
                                >
                                    {t('carContactHub')}
                                </button>
                            </div>
