...
import { BCA_CATEGORIES } from '../../category/lib/categoryConstants';
import { mergeBackendCategories } from '../../category/lib/categoryMegaMenu';
            ...
            <div className="w-full max-w-[1440px] mx-auto px-4 lg:px-8">
                <div className="flex items-center justify-between mb-3">
                    <div className="flex items-center gap-3">
                        <div className="w-1 h-7 bg-[#1CA0DB] rounded-full" />
                        <h2 className="text-lg sm:text-xl font-bold text-[#333]">
                            {t('browseByCategory') || 'Parcourir par Catégorie'}
                        </h2>
                    </div>
                    <Link to="/marketplace" className="flex items-center gap-1 text-[#333] text-sm font-semibold hover:text-[#1CA0DB] transition-colors shrink-0">
                        ...
                        <AnimatePresence mode="wait">
                            {isLoadingProds ? (
                                <motion.div key="loading" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
                                    ...
                                    <Loader2 className="size-7 text-[#1CA0DB] animate-spin" />
                                </motion.div>
                            ) : products.length === 0 ? (
                                <motion.div key="empty" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
                                    ...
                                    <button type="button" onClick={() => navigate('/marketplace')} className="text-[#1CA0DB] text-sm font-semibold hover:underline">
                                        {t('explore') || 'Explorer'} →
                                    </button>
                                </motion.div>
                            ) : (
