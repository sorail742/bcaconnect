...
import { BCA_ICONS, BCA_CATEGORIES } from '../../category/lib/categoryConstants';
                    ...
                    return (
                        <button
                            ...
                            className={cn(
                                ...
                                highlighted
                                    ? 'bg-[#eff6ff] text-[#1CA0DB]'
                                    : 'text-[#333] hover:bg-[#eff6ff] hover:text-[#1CA0DB]'
                            )}
                        >
                            <span className={cn('shrink-0 flex items-center justify-center', highlighted ? 'text-[#1CA0DB]' : 'text-[#333] group-hover:text-[#1CA0DB]')}>
                                <Icon className={cn('size-[22px]', highlighted ? 'text-[#1CA0DB]' : 'text-[#333] group-hover:text-[#1CA0DB]')} />
                            </span>
                            <span className={cn(
                                'flex-1 text-[14px] leading-snug pr-2',
                                highlighted ? 'font-semibold text-[#1CA0DB]' : 'font-normal text-[#333] group-hover:text-[#1CA0DB] group-hover:font-medium'
                            )}>
                                {cat.nom}
                            </span>
                            <ChevronRight className={cn(
                                'size-4 shrink-0 transition-colors',
                                highlighted ? 'text-[#1CA0DB]' : 'text-[#ccc] group-hover:text-[#1CA0DB]'
                            )} />
                        </button>
                    );
                })}
