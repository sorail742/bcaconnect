        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
          ...
          <div className="flex items-center gap-3 self-end sm:self-auto">
            <button
              ...
              className="flex items-center gap-2 h-9 px-4 bg-white border border-slate-200 text-slate-600 rounded-xl text-xs font-bold hover:border-[#1CA0DB] hover:text-[#1CA0DB] transition-all"
            >
            ...
            <button
              ...
              className="flex items-center gap-1.5 text-[#1CA0DB] font-bold text-sm hover:underline"
            >
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3 sm:gap-4">
            {trends.map((item, idx) => (
              <motion.button
                ...
                className="group text-left bg-white rounded-xl border border-slate-100 p-4 hover:shadow-md hover:border-[#1CA0DB]/30 transition-all duration-300 relative overflow-hidden"
              >
                <div className="flex items-start justify-between mb-3">
                  <div className="size-9 rounded-xl bg-blue-50 flex items-center justify-center group-hover:bg-[#1CA0DB] transition-colors">
                    <TrendingUp className="size-4 text-[#1CA0DB] group-hover:text-white transition-colors" />
                  </div>
                  <span className="text-[10px] font-black text-emerald-600 bg-emerald-50 px-2 py-0.5 rounded-full">
                    {item.growth}
                  </span>
                </div>
                <h3 className="text-xs sm:text-sm font-bold text-slate-800 leading-snug mb-2 line-clamp-2 group-hover:text-[#1CA0DB] transition-colors">
                  {item.label}
                </h3>
                <div className="flex items-center justify-between">
                  <span className="text-[10px] text-slate-400 font-medium">{item.volume}</span>
                  <ArrowUpRight className="size-3.5 text-slate-300 group-hover:text-[#1CA0DB] group-hover:rotate-45 transition-all" />
                </div>
                <span className="absolute top-2 right-2 text-[9px] font-black text-[#1CA0DB] bg-blue-50 px-1.5 py-0.5 rounded-full opacity-0 group-hover:opacity-100 transition-opacity">
                  {item.tag}
                </span>
                <div className="absolute inset-x-0 bottom-0 h-0.5 bg-[#1CA0DB] scale-x-0 group-hover:scale-x-100 transition-transform duration-300 origin-left" />
              </motion.button>
            ))}
          </div>
        ) : (
