...
export const aiService = {
    ...
    chat: async (message, conversationId) => {
        try {
            const response = await api.post('/ai/chat', { message, conversation_id: conversationId || undefined });
            return response.data;
        } catch (error) {
    ...
    /**
     * Historique réel des discussions IA de l'utilisateur connecté
     */
    ...
    /**
     * Messages complets d'une discussion IA (pour la recharger)
     */
    ...
    /**
     * Supprimer une discussion IA
     */
    deleteConversation: async (conversationId, confirmationText) => {
        const response = await api.delete(`/ai/conversations/${conversationId}`, { data: { confirmation_nom: confirmationText } });
        return response.data;
    },
    ...
    /**
     * Suggérer tous les détails d'un produit, prix inclus (Magic Fill)
     */
    ...
    /**
     * Suggérer les détails d'une ressource éducative
     */
    suggestEducationDetails: async (url, type) => {
        try {
            const response = await api.post('/ai/suggest-education-details', { url, type });
            return response.data;
        } catch (error) {
            console.error('Erreur suggestion éducation IA:', error);
            throw error;
        }
    },
