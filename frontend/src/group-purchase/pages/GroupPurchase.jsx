...
} from '../hooks/useGroupPurchaseData';
...
const CampaignCard = ({
  ...
  const remainingQty = Math.max(0, parseInt(campaign.quantite_cible, 10) - parseInt(campaign.quantite_actuelle, 10));
  const isExpired = campaign.date_limite && new Date(campaign.date_limite) < new Date();
  const canJoin = !isOrganizer && !participation && ['ouvert', 'atteint'].includes(campaign.statut) && !isExpired && remainingQty > 0;
    ...
    <div className="bg-card border border-border rounded-2xl p-5 flex flex-col gap-4 hover:shadow-lg transition-shadow">
      ...
      <div>
        <div className="flex justify-between text-xs font-bold mb-1">
          ...
          <span className={remainingQty <= 5 && remainingQty > 0 ? 'text-amber-500' : ''}>{pct}%</span>
        </div>
        <div className="h-2 bg-muted rounded-full overflow-hidden">
          <div
            className={`h-full rounded-full transition-all ${pct >= 100 ? 'bg-emerald-500' : pct >= 70 ? 'bg-amber-500' : 'bg-primary'}`}
            style={{ width: `${pct}%` }}
          />
        </div>
        <div className="flex justify-between text-xs mt-1">
          {remainingQty > 0 ? (
            <span className={`font-bold ${remainingQty <= 5 ? 'text-amber-500' : 'text-muted-foreground'}`}>
              {remainingQty <= 5 ? `⚠️ Plus que ${remainingQty} place(s) !` : `${remainingQty} places restantes`}
            </span>
          ) : (
            <span className="font-bold text-emerald-600">🎯 Objectif atteint !</span>
          )}
          {isExpired && <span className="font-bold text-destructive">Expiré</span>}
        </div>
      </div>
...
const JoinModal = ({ campaign, onClose, onConfirm, pending, walletBalance }) => {
  const remainingQty = Math.max(1, parseInt(campaign?.quantite_cible || 1, 10) - parseInt(campaign?.quantite_actuelle || 0, 10));
  ...
  const exceedsLimit = qty > remainingQty;
  ...
  return (
    <ModalOverlay open onClose={onClose} title="Rejoindre la campagne" maxWidth="max-w-md">
      <div className="px-6 pb-6">
        ...
        <div className="mb-4 px-3 py-2 bg-amber-50 border border-amber-200 rounded-xl text-xs font-bold text-amber-700">
          ⚡ Places restantes : {remainingQty}
        </div>
        <label className="block text-sm font-bold mb-2">Quantité (max: {remainingQty})</label>
        <input
          type="number"
          min={1}
          max={remainingQty}
          value={qty}
          onChange={(e) => setQty(Math.min(remainingQty, Math.max(1, parseInt(e.target.value, 10) || 1)))}
        ...
        <div className="flex gap-3">
          ...
          <button
            ...
            disabled={pending || insufficient || exceedsLimit}
