...
import { useTrustScore } from '../../ai/hooks/useAiData';
import { useMyStore, useVendorStats } from '../../shop/hooks/useVendorData';
import { useVendorOrders } from '../../order/hooks/useOrderData';
            ...
            <div className="min-h-screen bg-background p-6 lg:p-8 space-y-8 custom-scrollbar">
                ...
                <div className="flex items-center justify-between gap-4">
                    <div className="flex items-center gap-3">
                        <div className={cn(
                            ...
                            isOnline
                                ? "bg-emerald-50/50 dark:bg-emerald-500/10 border-emerald-100 dark:border-emerald-500/20 text-emerald-600 dark:text-emerald-400"
                                : "bg-amber-50/50 dark:bg-amber-500/10 border-amber-100 dark:border-amber-500/20 text-amber-600 dark:text-amber-400"
                        )}>
                    ...
                    <div className="flex flex-col xl:flex-row xl:items-center justify-between gap-6 relative z-10">
                        <div className="flex items-center gap-3">
                            ...
                            <div className="space-y-1 min-w-0">
                                <h2 className="text-xl font-black text-foreground uppercase tracking-tighter leading-none truncate max-w-[200px] sm:max-w-[400px]" style={{ fontFamily: "'Outfit', sans-serif" }}>
                        ...
                        <div className="flex items-center gap-3">
                             {store ? (
                                <button
                                    ...
                                    className="h-11 px-8 bg-slate-900 dark:bg-white text-white dark:text-slate-900 hover:bg-slate-800 dark:hover:bg-slate-100 rounded-2xl font-black text-[10px] uppercase tracking-widest transition-all shadow-xl shadow-slate-200 dark:shadow-black/30 flex items-center gap-3 group/btn border-none"
                                >
                        ...
                        kpis.map((kpi, idx) => (
                            <div key={idx} className="premium-card p-6 flex flex-col justify-between group/kpi h-32">
                                <div className="flex items-center justify-between">
                                    <div className="size-10 rounded-xl bg-muted flex items-center justify-center text-muted-foreground group-hover/kpi:scale-110 transition-transform duration-500">
                                        <kpi.icon className="size-5" />
                                    </div>
                                    <span className="text-[10px] font-bold text-emerald-600 dark:text-emerald-400 bg-emerald-50 dark:bg-emerald-500/10 px-2 py-0.5 rounded-lg">↑ {kpi.trendValue}</span>
                                </div>
                                <div className="space-y-1">
                                    <p className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest leading-none">{kpi.title}</p>
                                    <p className="text-xl font-black text-foreground tracking-tighter tabular-nums" style={{ fontFamily: "'Outfit', sans-serif" }}>{kpi.value.split(' ')[0]} <small className="text-xs opacity-50">{kpi.value.split(' ')[1] || ''}</small></p>
                                </div>
                            </div>
                        ))
                    )}
                ...
                <div className="grid grid-cols-1 lg:grid-cols-12 gap-3">
                    ...
                    <div className="lg:col-span-8 space-y-6">
                        ...
                        <HeroChartCard
                            title={t('vdFluxMonitor')}
                            subtitle={t('vdLast30Days')}
                            icon={TrendingUp}
                            data={statsData?.timeseries || []}
                            xKey="day"
                            yKey="val"
                            height={240}
                            emptyState={{ icon: TrendingUp, message: 'Pas encore de flux sur cette période' }}
                            headerExtra={
                                <div className="px-4 py-2 bg-muted text-muted-foreground rounded-xl text-[10px] font-black uppercase tracking-widest border border-border">
                                    {t('vdStatusOptimal')}
                                </div>
                            }
                            footer={
                                <div className="-mx-4 -mb-4 p-6 bg-muted/30 border-t border-border rounded-b-2xl flex flex-col sm:flex-row items-center justify-between gap-6">
                                    <div className="flex items-center gap-3">
                                        ...
                                        <div className="space-y-1">
                                            <p className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest leading-none">{t('vdAiTrend')}</p>
                                            <p className="text-xs font-black text-foreground uppercase">"{statsData?.global_trend?.toUpperCase() || t('vdStableTrend')}"</p>
                                        </div>
                                    </div>
                                    <Link to="/vendor/products" className="h-10 px-6 rounded-xl bg-card border border-border text-[10px] font-black uppercase tracking-widest text-muted-foreground hover:text-foreground hover:border-primary/30 transition-all flex items-center gap-3 group/link shadow-sm">
                                         {t('vdViewProducts')}
                        ...
                        <div className="premium-card overflow-hidden group/ledger">
                            <DataTable
                                ...
                                actions={<Link className="text-[10px] font-black text-primary uppercase tracking-widest hover:text-foreground transition-all px-6 py-4 flex items-center gap-2" to="/vendor/orders">{t('vdViewAll')} <ChevronRight className="size-3" /></Link>}
                            />
                        </div>
                    </div>
                    ...
                    <div className="lg:col-span-4 flex flex-col gap-3">
                        ...
                        <div className="premium-card p-6 flex flex-col h-fit group/supply relative overflow-hidden">
                            <div className="flex items-start justify-between mb-8">
                                <div className="space-y-1">
                                    <h3 className="text-xs font-black uppercase tracking-tight text-foreground" style={{ fontFamily: "'Outfit', sans-serif" }}>{t('vdStockResilience')}</h3>
                                    <p className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest leading-none">{t('vdAvailabilityAnalysis')}</p>
                                </div>
                            ...
                            <div className="space-y-6">
                                <div className="space-y-2">
                                    <div className="flex justify-between text-[10px] font-bold uppercase tracking-widest">
                                        <span className="text-muted-foreground">{t('vdAvailability')}</span>
                                        <span className="text-foreground">{productsCount > 0 ? Math.round(((productsCount - lowStockItems) / productsCount) * 100) : 0}%</span>
                                    </div>
                                ...
                                <div className="space-y-2">
                                    <div className="flex justify-between text-[10px] font-bold uppercase tracking-widest">
                                        <span className="text-muted-foreground">{t('vdStockAlert')}</span>
                            ...
                            <button
                                ...
                                className={cn(
                                    ...
                                    isAuditing ? "bg-muted text-muted-foreground/50 cursor-not-allowed" : "bg-slate-900 dark:bg-white text-white dark:text-slate-900 hover:bg-slate-800 dark:hover:bg-slate-100 shadow-xl shadow-slate-200 dark:shadow-black/30"
                                )}
                            >
                                {isAuditing ? (
                            ...
                            {aiRecommendation && (
                                <motion.div
                                    ...
                                    className="mt-4 p-4 bg-emerald-50 dark:bg-emerald-500/10 border border-emerald-100 dark:border-emerald-500/20 rounded-xl"
                                >
                                    <div className="flex items-center gap-2 mb-2">
                                        <div className="size-1.5 rounded-full bg-emerald-500 animate-pulse" />
                                        <span className="text-[9px] font-black text-emerald-600 dark:text-emerald-400 uppercase tracking-widest">{t('vdAiReport')}</span>
                                    </div>
                                    <p className="text-[10px] font-bold text-muted-foreground uppercase leading-relaxed">{aiRecommendation}</p>
                                </motion.div>
                            )}
                        </div>
                        ...
                        <div className="premium-card p-6 flex flex-col justify-between flex-1 group/trust relative overflow-hidden">
                            <div className="space-y-6 relative z-10">
                                <div className="flex items-center gap-3">
                                    <div className="size-10 rounded-xl bg-emerald-50 dark:bg-emerald-500/10 flex items-center justify-center text-emerald-500 border border-emerald-100 dark:border-emerald-500/20 group-hover/trust:rotate-12 transition-all duration-700">
                                        <ShieldCheck className="size-5" />
                                    </div>
                                    <div className="space-y-1">
                                        <h4 className="text-xs font-black uppercase tracking-tight text-foreground" style={{ fontFamily: "'Outfit', sans-serif" }}>{t('vdTrustScore')}</h4>
                                        <p className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest leading-none">{t('vdSupplierCert')}</p>
                                    </div>
                                </div>

                                <div className="space-y-2">
                                    <div className="flex items-center justify-between text-[10px] font-bold uppercase tracking-widest">
                                        <span className="text-muted-foreground">{t('vdIntegrity')}</span>
                            ...
                            <div className="mt-8 py-3 px-4 bg-emerald-50 dark:bg-emerald-500/10 text-[10px] font-black text-emerald-600 dark:text-emerald-400 uppercase tracking-widest rounded-xl text-center border border-emerald-100 dark:border-emerald-500/20">
                                {t('vdSecureTerminal')}
                            </div>
                        </div>
                    </div>
