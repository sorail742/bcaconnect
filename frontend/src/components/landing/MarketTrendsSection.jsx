...
export const MarketTrendsSection = () => {
  ...
  const getFallbackTrends = useCallback(
    () => [
      {
        label: t("trendAgriSolar"),
        growth: "+45%",
        volume: `1.2B ${t("gnf")}`,
        tag: t("tagTrend"),
      },
      {
        label: t("trendTextileLuxe"),
        growth: "+22%",
        volume: `850M ${t("gnf")}`,
        tag: t("tagPopular"),
      },
      {
        label: t("trendConstruction"),
        growth: "+38%",
        volume: `3.4B ${t("gnf")}`,
        tag: t("tagHighDemand"),
      },
      {
        label: t("trendElectronics"),
        growth: "+12%",
        volume: `2.1B ${t("gnf")}`,
        tag: t("tagLowPrice"),
      },
  ...
  const fetchTrends = async (showRefreshing = false) => {
    ...
    try {
      const [trendResult, statsResult] = await Promise.allSettled([
        aiService.getMarketTrends(),
        statService.getAdminStats(),
      ]);

      let liveTrends = false;
      if (trendResult.status === "fulfilled" && trendResult.value) {
        const raw =
          trendResult.value?.trends || trendResult.value?.data?.trends || [];
      ...
      if (statsResult.status === "fulfilled" && statsResult.value) {
        ...
        setStats({
          volume: statsData.totalVolume
            ...
            : getFallbackStats().volume,
          growth: statsData.growthRate
            ? `+${statsData.growthRate}%`
            : getFallbackStats().growth,
          active:
            statsData.totalProducts?.toString() || getFallbackStats().active,
        });
        setUsingFallback(false);
      } else {
        setStats(getFallbackStats());
        setUsingFallback(!liveTrends);
      }

      if (liveTrends || statsResult.status === "fulfilled") {
              ...
              <p className="text-xs text-slate-400 flex items-center gap-1.5 mt-0.5">
                ...
                {usingFallback && (
                  <span className="text-amber-600">
                    • {t("demoData") || "Données indicatives"}
                  </span>
                )}
                {lastUpdated && (
        ...
        <div className="flex flex-wrap gap-3 mb-6">
          {[
            ...
            >
              ...
              <span className="font-black tabular-nums auto-scale-text leading-none">
                {s.val}
              </span>
            </div>
          ))}
