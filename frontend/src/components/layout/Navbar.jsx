...
import {
    ...
    MapPin, Bell, LayoutDashboard, Package, Cog, LogOut, Heart, Wallet
} from 'lucide-react';
        ...
        <header className="w-full font-sans bg-white border-b border-gray-200">
            ...
            <div className="bg-gradient-to-r from-[#b5f5ec] via-[#87e8de] to-[#0050b3] text-[#0050b3] h-10 flex items-center justify-center text-sm font-semibold relative overflow-hidden">
                <div translate="no" className="flex items-center gap-4 z-10">
                    <span className="font-extrabold text-black">BCA Work</span>
                    <span className="text-gray-400">|</span>
                    <span className="flex-1 text-black">🤖 Une équipe d'agents d'IA à votre service, 24/7</span>
                    <Link to="/register" className="ml-4 underline font-bold text-white">Essai gratuit</Link>
                </div>
                {/* Decorative slant */}
                <div className="absolute right-0 top-0 bottom-0 w-1/3 bg-[#0050b3] skew-x-12 translate-x-10 z-0"></div>
            ...
            <div className="container mx-auto px-4 lg:px-8 py-4 lg:py-5 flex items-center gap-4 lg:gap-6">
                ...
                <div className="hidden md:flex flex-1 min-w-0 max-w-2xl relative group">
                    <form onSubmit={handleSearch} className="w-full flex items-center rounded-full border-2 border-[#FF6600] bg-white transition-shadow hover:shadow-md focus-within:shadow-md h-12">
                        <input
                            type="text"
                            placeholder="Que recherchez-vous ?"
                            className="flex-1 h-full pl-5 pr-2 text-sm text-gray-900 bg-transparent outline-none placeholder:text-gray-400 min-w-0"
                            value={searchQuery}
                ...
                <div className="flex items-center gap-3 lg:gap-4 shrink-0 ml-auto">
                    ...
                    <div className="hidden xl:flex flex-col cursor-pointer group">
                        <span className="text-[11px] text-gray-500">Adresse de livraison :</span>
                        <div className="flex items-center gap-1.5 font-bold text-sm text-gray-900 group-hover:text-[#FF6600] transition-colors">
                    ...
                    <button onClick={() => changeLanguage(lang === 'FR' ? 'EN' : 'FR')} className="hidden lg:flex items-center gap-2 cursor-pointer group">
                        <Globe className="size-5 text-gray-700 group-hover:text-[#FF6600] transition-colors" />
                        <span className="font-bold text-sm text-gray-900 group-hover:text-[#FF6600] transition-colors">Français-GNF</span>
                    </button>

                    {/* Cart */}
                    <Link to="/cart" className="relative flex items-center group cursor-pointer">
                        <ShoppingCart className="size-6 text-gray-700 group-hover:text-[#FF6600] transition-colors" />
                        {cartCount > 0 && (
                            <span className="absolute -top-1.5 -right-2 bg-[#FF6600] text-white text-[10px] font-bold px-1.5 rounded-full border-2 border-white">
                                {cartCount}
                            </span>
                        )}
                    </Link>
                    ...
                    {user ? (
                        ...
                        >
                            <Link to="/dashboard" className="flex items-center gap-2 group cursor-pointer">
                                <User className="size-6 text-gray-700 group-hover:text-[#FF6600] transition-colors" />
                                <div className="hidden lg:block text-left">
                                    <span className="block text-[11px] text-gray-500">Bonjour, {user.nom_complet?.split(' ')[0]}</span>
                                    <span className="block font-bold text-sm text-gray-900 group-hover:text-[#FF6600] transition-colors">Mon Compte</span>
                                </div>
                            </Link>
                            ...
                            <AnimatePresence>
                                {isUserMenuOpen && (
                                    <motion.div
                                        ...
                                        exit={{ opacity: 0, y: 10 }}
                                        className="absolute top-full right-0 mt-4 w-60 bg-white border border-gray-200 rounded-xl shadow-xl z-50 p-4"
                                    >
                                        <div className="mb-4">
                                            <p className="font-bold text-gray-900">{user.nom_complet}</p>
                        ...
                        <div className="flex items-center gap-3 shrink-0">
                            <Link to="/login" className="flex items-center gap-1.5 group whitespace-nowrap">
                                <User className="size-5 text-gray-700 group-hover:text-[#FF6600] transition-colors shrink-0" />
                                <span className="hidden md:block font-bold text-sm text-gray-900 group-hover:text-[#FF6600] transition-colors">Se connecter</span>
                            </Link>
                            <Link to="/register" className="flex items-center bg-[#FF6600] text-white px-4 py-2 rounded-full font-bold text-sm hover:bg-[#e65c00] transition-colors shadow-md whitespace-nowrap shrink-0">
                    ...
                    {/* Mobile Menu Toggle */}
                    <button onClick={() => setIsMenuOpen(true)} className="md:hidden text-gray-900">
            ...
            <div className="md:hidden px-4 pb-4">
                <form onSubmit={handleSearch} className="w-full flex items-center rounded-full border border-[#FF6600] bg-white h-10 overflow-hidden">
                    <input
                        type="text"
                ...
                {isMenuOpen && (
                    <>
                        ...
                        <motion.div
                            ...
                            className="fixed top-0 left-0 bottom-0 w-[85%] bg-white z-50 flex flex-col md:hidden overflow-y-auto"
                        >
                            <div className="bg-[#FF6600] p-6 text-white flex items-center justify-between">
                            ...
                            <div className="flex-1 p-4 flex flex-col gap-6">
                                {!user && (
                                    <div className="flex gap-2">
                                        <Link to="/register" onClick={() => setIsMenuOpen(false)} className="flex-1 text-center bg-[#FF6600] text-white py-2 rounded-full font-bold text-sm">S'inscrire</Link>
                                        <Link to="/login" onClick={() => setIsMenuOpen(false)} className="flex-1 text-center bg-gray-100 text-gray-800 py-2 rounded-full font-bold text-sm">Se connecter</Link>
                                    </div>
                                )}

                                <div className="flex flex-col gap-4">
                                    <h4 className="font-bold text-gray-900 border-b pb-2">Catégories Plébiscitées</h4>
                                    <Link to="/marketplace" onClick={() => setIsMenuOpen(false)} className="text-gray-600 hover:text-[#FF6600]">Tous les produits</Link>
                                    <Link to="/vendors" onClick={() => setIsMenuOpen(false)} className="text-gray-600 hover:text-[#FF6600]">Vendeurs Locaux</Link>
                                    <Link to="/marketplace?filter=flash" onClick={() => setIsMenuOpen(false)} className="text-gray-600 hover:text-[#FF6600]">Ventes Flash</Link>
                                </div>

                                <div className="flex flex-col gap-4">
                                    <h4 className="font-bold text-gray-900 border-b pb-2">Services & Aide</h4>
                                    <Link to="/help" onClick={() => setIsMenuOpen(false)} className="text-gray-600 hover:text-[#FF6600]">Protection des commandes</Link>
                                    <Link to="/help" onClick={() => setIsMenuOpen(false)} className="text-gray-600 hover:text-[#FF6600]">Centre d'assistance</Link>
                                </div>
                            </div>
                        </motion.div>
                    </>
