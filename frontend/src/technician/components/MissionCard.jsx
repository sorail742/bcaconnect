...
import {
    MapPin, Phone, Wrench, CheckCircle2, AlertCircle, Loader2, FileText, ChevronRight, GraduationCap,
} from 'lucide-react';
                ...
                <div className="flex gap-4 flex-1">
                    ...
                    <div className="space-y-2 flex-1 min-w-0">
                        <div className="flex flex-wrap items-center gap-2">
                            ...
                            {mission.specialite && (
                                <span className="flex items-center gap-1 px-2.5 py-0.5 rounded-full text-[10px] font-bold uppercase bg-primary/10 text-primary">
                                    <GraduationCap className="size-3" /> {mission.specialite}
                                </span>
                            )}
                        </div>
                        {!compact && (
                        ...
                        {/* Photos du problème client */}
                        {!compact && mission.photos_urls?.length > 0 && (
                            <div className="flex gap-2 flex-wrap">
                                {mission.photos_urls.map((url, i) => (
                                    <a
                                        ...
                                        className="block size-14 rounded-lg overflow-hidden border border-border hover:border-primary/50 transition-colors shrink-0"
                                    >
                                        <img
