...
const Returns = () => {
    ...
    const handleExportPDF = () => {
        ...
        const headerY = addPdfLetterhead(doc, "AUDIT DES RETOURS ET LITIGES", `REGISTRE LOGISTIQUE • GÉNÉRÉ LE: ${new Date().toLocaleString()} • TOTAL: ${filtered.length}`);
    ...
    return (
        <DashboardLayout title="GESTION DES RETOURS" noPadding>
            <div className="min-h-screen bg-background p-6 lg:p-12 space-y-12 animate-in pb-24 custom-scrollbar">

                {/* Command Center - Premium Header */}
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-8">
                    <div className="flex items-start gap-6">
                        <div className="size-12 rounded-2xl bg-amber-500/10 border border-amber-500/20 flex items-center justify-center shadow-2xl shadow-amber-500/10">
                    ...
                    <div className="flex items-center gap-4">
                        <div className="relative group">
                            <Search className="absolute left-6 top-1/2 -translate-y-1/2 size-5 text-muted-foreground group-focus-within:text-amber-500 transition-colors" />
                            <input 
                                value={search}
                                onChange={e => setSearch(e.target.value)}
                                placeholder="IDENTIFICATION RETOUR / NOM..."
                                className="h-12 w-80 bg-card border border-border rounded-2xl pl-16 pr-8 text-[11px] font-black uppercase tracking-widest outline-none transition-all placeholder:text-muted-foreground/40 focus:ring-4 focus:ring-amber-500/5 shadow-sm"
                            />
                        </div>
                        <button 
                        ...
                        <button 
                            onClick={() => refetch()} 
                            disabled={loading}
                            className="size-12 rounded-2xl bg-card border border-border flex items-center justify-center hover:bg-muted transition-all shadow-xl shadow-slate-200/40 disabled:opacity-50"
                        >
                        ...
                        >
                            <div className="bg-card p-8 rounded-3xl border border-primary/20 bg-primary/[0.01] grid grid-cols-1 md:grid-cols-4 gap-6 shadow-2xl shadow-primary/5">
                                <button onClick={handleExportExcel} className="p-8 rounded-3xl bg-card border border-border hover:border-emerald-200 hover:shadow-xl hover:shadow-emerald-500/5 flex flex-col items-center gap-4 transition-all group">
                                    <div className="size-11 rounded-2xl bg-emerald-50 dark:bg-emerald-500/10 flex items-center justify-center group-hover:scale-110 transition-transform">
                                    ...
                                    <div className="text-center">
                                        ...
                                        <p className="text-[9px] font-bold text-muted-foreground mt-1">Logistics Ledger .xlsx</p>
                                    </div>
                                </button>
                                <button onClick={handleExportPDF} className="p-8 rounded-3xl bg-card border border-border hover:border-rose-200 hover:shadow-xl hover:shadow-rose-500/5 flex flex-col items-center gap-4 transition-all group">
                                    <div className="size-11 rounded-2xl bg-rose-50 dark:bg-rose-500/10 flex items-center justify-center group-hover:scale-110 transition-transform">
                                    ...
                                    <div className="text-center">
                                        ...
                                        <p className="text-[9px] font-bold text-muted-foreground mt-1">Audit Report .pdf</p>
                                    </div>
                                </button>
                                <button onClick={handleExportCSV} className="p-8 rounded-3xl bg-card border border-border hover:border-blue-200 hover:shadow-xl hover:shadow-blue-500/5 flex flex-col items-center gap-4 transition-all group">
                                    <div className="size-11 rounded-2xl bg-blue-50 dark:bg-blue-500/10 flex items-center justify-center group-hover:scale-110 transition-transform">
                                    ...
                                    <div className="text-center">
                                        ...
                                        <p className="text-[9px] font-bold text-muted-foreground mt-1">Raw Logistics Data .csv</p>
                                    </div>
                                </button>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                    {loading ? (
                        Array(4).fill(0).map((_, n) => (
                            <div key={n} className="h-96 bg-card border border-border rounded-3xl animate-pulse" />
                        ))
                    ) : filtered.length === 0 ? (
                        <div className="lg:col-span-2 py-32 bg-card rounded-3xl border-2 border-dashed border-border flex flex-col items-center justify-center gap-6 opacity-40 text-center">
                            <ArrowRightLeft className="size-12 text-muted-foreground" />
                            <p className="text-xl font-black uppercase tracking-tighter text-muted-foreground">Aucun signal de retour actif</p>
                        </div>
                    ) : (
                        filtered.map(item => (
                            <motion.div
                                ...
                                key={item.id}
                                className="bg-card p-10 rounded-3xl border border-border shadow-xl shadow-slate-200/30 group relative overflow-hidden transition-all duration-500 hover:shadow-2xl hover:shadow-amber-500/5 hover:-translate-y-1"
                            >
                                ...
                                <div className="space-y-10">
                                    <div className="flex items-center gap-6">
                                        <div className="size-12 rounded-2xl bg-muted border border-border flex items-center justify-center relative overflow-hidden group-hover:border-amber-500/30 transition-all duration-700">
                                            ...
                                            <Package className="size-8 text-muted-foreground group-hover:text-amber-500 transition-colors" />
                                        </div>
                                        <div className="space-y-1">
                                            <p className="text-lg font-black text-foreground uppercase tracking-tighter leading-none" style={{ fontFamily: "'Outfit', sans-serif" }}>
                                                {item.demandeur?.nom_complet || "Client inconnu"}
                                            </p>
                                            <p className="text-[10px] font-black text-muted-foreground uppercase tracking-widest leading-none">Transmission Node: #{item.id?.slice(0, 8).toUpperCase()}</p>
                                        </div>
                                    </div>

                                    <div className="p-8 bg-muted/50 rounded-3xl border border-border space-y-6 group-hover:border-amber-500/10 transition-colors">
                                        <div className="flex items-start gap-4">
                                            <ShieldAlert className="size-5 text-amber-500 shrink-0 mt-1" />
                                            <div className="space-y-2">
                                                <p className="text-[9px] font-black text-muted-foreground uppercase tracking-[0.2em] leading-none">Diagnostic Motif</p>
                                                <p className="text-sm text-foreground/80 font-black uppercase leading-relaxed tracking-tight group-hover:text-foreground transition-colors line-clamp-3">
                                        ...
                                        <div className="pt-6 border-t border-border grid grid-cols-2 gap-4">
                                            <div className="flex items-center gap-3">
                                                ...
                                                <span className="text-[10px] font-black text-muted-foreground uppercase tracking-widest tabular-nums">{new Date(item.createdAt).toLocaleDateString('fr-GN')}</span>
                                            </div>
                                            <div className="flex items-center gap-3">
                                                <History className="size-4 text-muted-foreground" />
                                                <span className="text-[10px] font-black text-muted-foreground uppercase tracking-widest">Quantum_Sync</span>
                                            </div>
                                        </div>
                                    </div>
                                    ...
                                    {item.statut === 'ouvert' && (
                                        <div className="flex gap-4 pt-4">
                                            <button
                                                ...
                                                className="flex-1 h-14 bg-muted border border-border text-rose-500 rounded-2xl text-[10px] font-black uppercase tracking-[0.2em] hover:bg-rose-50 dark:hover:bg-rose-500/10 hover:border-rose-200 dark:hover:border-rose-500/30 transition-all flex items-center justify-center gap-3 shadow-sm disabled:opacity-50"
                                            >
                                            ...
                                            <button
                                                ...
                                                disabled={isResolving}
                                                className="flex-1 h-14 bg-slate-900 dark:bg-white text-white dark:text-slate-900 rounded-2xl text-[10px] font-black uppercase tracking-[0.2em] hover:bg-amber-500 hover:text-white transition-all flex items-center justify-center gap-3 shadow-xl shadow-slate-200 disabled:opacity-50"
                                            >
